...
export function FilterBar({ filters }: { filters: FilterSpec[] }) {
  ...
  const inputCls =
    'border border-panel-300 bg-surface-raised px-3 py-2 text-sm text-ink-900 ' +
    ...
    <div className="mb-4 flex flex-wrap items-end gap-3 border border-panel-200 bg-surface-raised p-4">
      ...
      {activeCount > 0 && (
        <button
          type="button"
          onClick={clear}
          className="flex cursor-pointer items-center gap-1.5 border border-transparent px-3 py-2 text-sm font-medium text-panel-600 transition-colors duration-150 hover:border-panel-200 hover:bg-panel-50 hover:text-ink-800 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-signal-600"
        >
