...
function Ranked({
  ...
  return (
    <ul className="space-y-2">
      {items.map((item) => (
        <li key={item.key} className="rounded-lg border border-panel-200 bg-white px-4 py-3">
        ...
        {data.knowledgeGaps.length === 0 ? (
          ...
          <ul className="space-y-2">
            {data.knowledgeGaps.map((gap) => (
              <li
                key={gap.id}
                className="flex flex-wrap items-start justify-between gap-3 rounded-lg border border-panel-200 bg-white px-4 py-3"
              >
        ...
        {data.peakTimes.length === 0 ? (
          ...
          <ul className="flex flex-wrap gap-2">
            {data.peakTimes.map((p) => (
              <li
                key={`${p.dow}-${p.hour}`}
                className="rounded-lg border border-panel-200 bg-white px-3 py-2 text-xs"
